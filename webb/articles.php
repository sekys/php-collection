...
if (!isset($cat_id)) {
    ...
    $result = DB::Query("SELECT * FROM ".DB_PREFIX."article_cats WHERE ".groupaccess('article_cat_access')." ORDER BY article_cat_name");
    $rows = dbrows($result);
    if ($rows != 0) {
        $counter = 0; $columns = 2;
    ...
    $result = DB::Query("SELECT * FROM ".DB_PREFIX."article_cats WHERE article_cat_id='$cat_id'");
    if (dbrows($result) != 0) {
        ...
        if (checkgroup($cdata['article_cat_access'])) {
            ...
            if ($rows != 0) {
                $result = DB::Query("SELECT * FROM ".DB_PREFIX."articles WHERE article_cat='$cat_id' ORDER BY ".$cdata['article_cat_sorting']." LIMIT $rowstart,15");
                $numrows = dbrows($result); $i = 1;
                while ($data = dbarray($result)) {
                    if ($data['article_datestamp']+604800 > Time::$TIME+($settings['Time::$TIMEoffset']*3600)) {
