...
{
	$temp = explode("-", trim($_POST['datum']));
	$cas = mktime(0, 0, 0, $temp[1], $temp[2], $temp[0]);
	$rozdiel = $cas+60*60*24;
	$prikaz = "`datum` > '".DB::Vstup($cas)."' AND `datum` < '".DB::Vstup(trim($rozdiel))."'";
	...
	$z->celkovo = SLS::Count2("SELECT COUNT(id) as pocet FROM `phpbanlist`.`acp_vyzva` 
										WHERE prijal IS NULL AND sukromna = 0 AND
											(( ".$prikaz." )
											OR `mapa` LIKE '%".$mapa."%')");						
} else {
	@$sql_vyzva=SLS::Query2("SELECT * FROM `phpbanlist`.`acp_vyzva` c
								...
								ORDER BY datum ".$z->mysql()."");
	$z->celkovo = SLS::Count2("SELECT COUNT(id) as pocet FROM `phpbanlist`.`acp_vyzva` WHERE prijal IS NULL AND sukromna = 0");
}
			...
			echo '
				<tr>
					<td class="cup_riadok" width="10" align="center" style="padding: 0px;">
						<a href="', SLSPlugin::Adresa(10), $datum, '/'.$hodina.'/'.$vyzva['server'].'/">
					...
					<td class="cup_riadok" width="100" align="center" >
						<a href="', SLSPlugin::Adresa(5), $vyzva['ziada'], '/">', SLSClan::ClanMeno($vyzva['meno']), '</a> 
					</td>	
