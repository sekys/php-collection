...
$z->get('p1');
$z->list = 20;
$z->celkovo = SLS::Count2("SELECT COUNT(id) as pocet FROM `phpbanlist`.`acp_clans` WHERE volne ='1'");
...
if( $z->celkovo ) {
	...
	@$sql = SLS::Query2("SELECT id, meno, bodov, COALESCE(hracov,0) as hracov, volne FROM `phpbanlist`.`acp_clans` c
							LEFT JOIN ( SELECT clan_id, COUNT(user_id) AS hracov FROM `cstrike`.`fusion_users` GROUP BY clan_id ) h
								ON c.id = h.clan_id
							JOIN ( SELECT clan_id, user_id AS leader FROM `cstrike`.`fusion_users` WHERE `hodnost`='1' LIMIT 1 ) h
								ON c.id = h.clan_id									
						WHERE volne ='1' ORDER BY hracov desc ".$z->mysql().""
						);
					
	while($row=mysqli_fetch_assoc($sql)) // FindLeader cez LEFT JOIN
	{ 
		echo '<tr> 
				<td class="cup_riadok" width="30" align="center">
					<a href="'.ROOT.'messages.php?msg_send='.$row['leader'].'">
