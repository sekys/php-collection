...
$z->get('p1');
$z->list = 25;
$z->celkovo = SLS::Count2("SELECT COUNT(id) as pocet FROM `phpbanlist`.`acp_clans`");
...
if( $z->celkovo ) 
{
	...
	*/		
	@$sql = SLS::Query2("SELECT id, meno, bodov FROM `phpbanlist`.`acp_clans` ORDER BY bodov desc ".$z->mysql()."");		
	$rank = SLS::$zoznam;
