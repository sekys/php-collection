...
if (isset($cat_id)) {
	$res = 0;
	$result = DB::Query("SELECT * FROM ".$db_prefix."news_cats WHERE news_cat_id='$cat_id'");
	if (dbrows($result) || $cat_id == 0) {
		$data = dbarray($result);
		$rows = dbcount("(news_id)", "news", "news_cat='$cat_id' AND ".groupaccess('news_visibility')." AND (news_start='0'||news_start<=".Time::$TIME.") AND (news_end='0'||news_end>=".Time::$TIME.")");
		if ($rows) {
			$res = 1;
			...
			$result2 = DB::Query("SELECT * FROM ".$db_prefix."news WHERE news_cat='$cat_id' AND ".groupaccess('news_visibility')." AND (news_start='0'||news_start<=".Time::$TIME.") AND (news_end='0'||news_end>=".Time::$TIME.") ORDER BY news_datestamp DESC");
	...
	$res = 0;
	$result = DB::Query("SELECT * FROM ".$db_prefix."news_cats ORDER BY news_cat_id");
	if (dbrows($result)) {
		echo "<table cellpadding='0' cellspacing='1' width='100%' class='tbl-border'>\n";
		while ($data = dbarray($result)) {
			$rows = dbcount("(news_id)", "news", "news_cat='".$data['news_cat_id']."' AND ".groupaccess('news_visibility')." AND (news_start='0'||news_start<=".Time::$TIME.") AND (news_end='0'||news_end>=".Time::$TIME.")");
			echo "<tr>\n<td width='150' class='tbl1' style='vertical-align:top'><img src='".IMAGES_NC.$data['news_cat_image']."' alt='".$data['news_cat_name']."'><br><br>\n";
			echo "<b>".$locale['401']."</b> ".$data['news_cat_name']."<br>\n<b>".$locale['402']."</b> $rows</td>\n";
			echo "<td class='tbl1' style='vertical-align:top'>\n";
			if ($rows) {
				$result2 = DB::Query("SELECT * FROM ".$db_prefix."news WHERE news_cat='".$data['news_cat_id']."' AND ".groupaccess('news_visibility')." AND (news_start='0'||news_start<=".Time::$TIME.") AND (news_end='0'||news_end>=".Time::$TIME.") ORDER BY news_datestamp DESC LIMIT 10");
				while ($data2 = dbarray($result2)) {
					echo "<img src='".THEME."images/bullet.gif' alt=''> <a href='news.php?readmore=".$data2['news_id']."'>".$data2['news_subject']."</a><br>\n";
				}
	...
	$result = DB::Query("SELECT * FROM ".$db_prefix."news WHERE news_cat='0' AND ".groupaccess('news_visibility')." AND (news_start='0'||news_start<=".Time::$TIME.") AND (news_end='0'||news_end>=".Time::$TIME.") ORDER BY news_datestamp DESC LIMIT 10");
	if (dbrows($result)) {
		if ($res == 0) echo "<table cellpadding='0' cellspacing='1' width='100%' class='tbl-border'>\n";
		$nrows = dbcount("(news_id)", "news", "news_cat='0' AND ".groupaccess('news_visibility')." AND (news_start='0'||news_start<=".Time::$TIME.") AND (news_end='0'||news_end>=".Time::$TIME.")");
