...
if (iMEMBER) {
	...
	$result = DB::Query("SELECT * FROM ".$db_prefix."users".$orderby."");
	$rows = dbrows($result);
...
<td align='center' width='1%' class='tbl2' style='white-space:nowrap'><b>".$locale['402']."</b></td>
		...
		$result = DB::Query("SELECT * FROM ".$db_prefix."users".$orderby." ORDER BY user_level DESC, user_name LIMIT $rowstart,20");
